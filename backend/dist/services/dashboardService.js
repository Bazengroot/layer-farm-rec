...
class DashboardService {
     ...
     */
    async getFarmManagerDashboard(farmId) {
        let flockQuery = supabaseAdmin_1.supabaseAdmin.from('flocks').select('id, name, initial_count, current_count, status');
        if (farmId)
            flockQuery = flockQuery.eq('farm_id', farmId);
        ...
        const { data: eggRecords } = await supabaseAdmin_1.supabaseAdmin
            .from('daily_egg_production')
            .select('good_eggs_count, cracked_eggs_count, dirty_eggs_count, broken_eggs_count')
            .limit(100);
        const totalEggs = (eggRecords || []).reduce((sum, r) => sum + (r.good_eggs_count + r.cracked_eggs_count + r.dirty_eggs_count + r.broken_eggs_count), 0);
        // Mortality & Feed
        const { data: healthRecords } = await supabaseAdmin_1.supabaseAdmin.from('daily_health_records').select('mortality_count').limit(100);
        const totalMortality = (healthRecords || []).reduce((sum, r) => sum + (r.mortality_count || 0), 0);
        const { data: feedRecords } = await supabaseAdmin_1.supabaseAdmin.from('daily_feed_consumption').select('quantity_kg').limit(100);
