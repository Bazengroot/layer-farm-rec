...
class PerformanceKPIService {
     ...
     */
    async computeFlockKPIs(flockId, startDateStr, endDateStr) {
        const kpis = [];
        // 1. Fetch Flock details with House and Farm relations
        const { data: flock, error: flockErr } = await supabaseAdmin_1.supabaseAdmin
            .from('flocks')
        ...
        const { data: eggData } = await supabaseAdmin_1.supabaseAdmin.rpc('get_flock_total_eggs', {
            p_flock_id: flockId,
            p_start_date: startDateStr,
            p_end_date: endDateStr,
        });
        const totalEggs = Number(eggData || 0);
        const { data: mortData } = await supabaseAdmin_1.supabaseAdmin.rpc('get_flock_total_mortality', {
            p_flock_id: flockId,
            p_start_date: startDateStr,
            p_end_date: endDateStr,
        });
        const totalMortality = Number(mortData || 0);
        const { data: cullData } = await supabaseAdmin_1.supabaseAdmin.rpc('get_flock_total_culling', {
            p_flock_id: flockId,
            p_start_date: startDateStr,
            p_end_date: endDateStr,
        });
        const totalCulling = Number(cullData || 0);
        const { data: feedKgData } = await supabaseAdmin_1.supabaseAdmin.rpc('get_flock_total_feed_kg', {
            p_flock_id: flockId,
            p_start_date: startDateStr,
            p_end_date: endDateStr,
        });
        const totalFeedKg = Number(feedKgData || 0);
        const { data: eggWeightKgData } = await supabaseAdmin_1.supabaseAdmin.rpc('get_flock_total_egg_weight_kg', {
            p_flock_id: flockId,
            p_start_date: startDateStr,
            p_end_date: endDateStr,
        });
        const totalEggWeightKg = Number(eggWeightKgData || 0);
        const { data: feedCostData } = await supabaseAdmin_1.supabaseAdmin.rpc('get_flock_total_feed_cost', {
            p_flock_id: flockId,
            p_start_date: startDateStr,
            p_end_date: endDateStr,
        });
        ...
        // Livability (%)
        const livabilityWarnings = [];
        let livabilityValue = null;
        if (initialHens <= 0) {
        ...
        const eggWeightWarnings = [];
        let avgEggWeightValue = null;
        if (totalEggs <= 0) {
        ...
        const feedBirdWarnings = [];
        let feedPerBirdValue = null;
        if (avgLiveHens <= 0) {
        ...
        // Feed Cost per Egg
        const costEggWarnings = [];
        let costPerEggValue = null;
        if (totalEggs <= 0) {
        ...
        const costMassWarnings = [];
        let costPerKgMassValue = null;
        if (totalEggWeightKg <= 0) {
...
exports.PerformanceKPIService = PerformanceKPIService;
exports.performanceService = new PerformanceKPIService();
//# sourceMappingURL=performanceKPIService.js.map